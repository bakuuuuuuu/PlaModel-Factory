    ...
    useEffect(() => {
        const fetchRecommendations = async () => {
            try {
                ...
                const newProducts = products.filter(product => product.isNewProduct == 1);
            ...
            <div className="homeContainer">
                ...
                <div>
                    <div className="home-new-products">
                        {newProductRecommendations.map((product, index) => (
                            <div key={index} className="new-product-item" onClick={() => handleProductClick(product.id)}>
                                <img src={getAbsoluteImageUrl(product.image)} alt={`New Product ${index + 1}`} id="home-new-product-img" />
                                <p>{product.name}</p>
                                <p>{new Intl.NumberFormat('ko-KR').format(product.new_price)}원</p>
                            </div>
                        ))}
                    </div>
                </div>
                <div className="home-recommendation-category">
                    <div className="home-recommendation-category-pokemon" onClick={() => handleCategoryClick('pokemon')}>
                    </div>
                    <div className="home-recommendation-category-digimon" onClick={() => handleCategoryClick('digimon')}>
                    </div>
                    <div className="home-recommendation-category-gundam" onClick={() => handleCategoryClick('gundam')}>
                    </div>
                    <div className="home-recommendation-category-hexagear" onClick={() => handleCategoryClick('hexagear')}>
                    </div>
                </div>
                ...
                <div className="home-recommendation">
                    {recommendations.map((product, index) => (
                        <div key={index} className="recommendation-item" onClick={() => handleProductClick(product.id)}>
                            <img src={getAbsoluteImageUrl(product.image)} alt={`Recommendation ${index + 1}`} id="home-recommendation-img" />
                            <p>{product.name}</p>
                            <p>{new Intl.NumberFormat('ko-KR').format(product.new_price)}원</p>
                        </div>
                    ))}
